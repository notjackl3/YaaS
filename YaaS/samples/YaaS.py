...
def upload(main_folder, title) -> str:
    uploader = Uploader()
    print(f"Youtube video name: {title}")
    uploaded_link = uploader.upload_video(f"{main_folder}/output.mp4", title, "Youtube as a service. By Jack Le.")
    return uploaded_link


def process(main_folder, video_object: YaaSObject):
    chunks_length = len(video_object.output_chunks)

    downloader = Downloader(main_folder, video_object.name)
    downloader.download_video(video_object.yt_video_link)

    with tempfile.TemporaryDirectory() as returning_chunks_folder:
        vid_cap = cv2.VideoCapture(f'{main_folder}/{video_object.name}.mp4')
        ...
        with open(f"{main_folder}/restored-{os.path.basename(video_object.original_file)}", "wb") as file2:
            buffer = io.BufferedWriter(file2)
            buffer.write(bytearray(bytes_list))
            buffer.flush()


def create_video(main_folder, chunks_folder, file_name: str, title: str, existing_link: Optional[str] = None):
    ...
    if not existing_link:
        ...
        obj.yt_video_link = upload(main_folder, obj.name)
    else:
...
def main_function():
    ...
    with tempfile.TemporaryDirectory() as main_folder:
        with tempfile.TemporaryDirectory() as chunks_folder:
            existing_youtube_link = "https://youtu.be/vJMPMYK6rT8"
            video_object, video_id = create_video(main_folder, chunks_folder, file_to_compress, video_name, existing_youtube_link)
            ...
            print(os.path.isfile(f"{main_folder}/restored-{os.path.basename(video_object.original_file)}"))
            dst = os.path.join(os.getcwd(), f"restored-{os.path.basename(video_object.original_file)}")
            shutil.copyfile(f"{main_folder}/restored-{os.path.basename(video_object.original_file)}", dst)
